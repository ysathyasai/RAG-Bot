...
async def chat(req: ChatRequest):
    ...
    if req.stream:
        async def event_stream():
            async for chunk in rag_engine.chat_stream(req.message, req.history or []):
    ...
    result = rag_engine.chat(req.message, req.history or [])
    return result
...
if __name__ == "__main__":
    ...
    reload_mode = os.getenv("RELOAD", "false").lower() in {"1", "true", "yes", "on"}
